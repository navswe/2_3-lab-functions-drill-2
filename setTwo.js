...
const first = (array, callbackFunction2) => callbackFunction2(array[0]);
...
const last = (array, callbackFunction3) =>
  callbackFunction3(array[array.length - 1]);
...
const contains = function (array, name, callbackFunction4) {
  if (array.includes(name, 0)) {
    callbackFunction4(true);
  } else {
    callbackFunction4(false);
  }
};
...
const uniq = (array, callbackFunction5) => {
  let arrayCopy = [...array]; // makes a copy of array, this will be modified later with splice()
  let dupName = [];
  let dupArray = []; // array storing duplicates
  for (let i = 0; i < arrayCopy.length; i++) {
    for (let j = i + 1; j < arrayCopy.length; j++) {
      if (arrayCopy[i] === arrayCopy[j]) {
        dupName = arrayCopy.splice(j, 1);
        dupArray.push(dupName);
      }
    }
  }
  callbackFunction5(arrayCopy, dupArray); // returns only unique elements in array.
};
...
uniq(names, (uniqArr, duplicates) =>
  console.log(
    `The new names array with all the duplicate items removed is ${uniqArr}. The duplicates array is ${duplicates}.`
  )
);
...
const each = function (array, callbackFunction6) {
  for (let i = 0; i < array.length; i++) {
    callbackFunction6(array[i], i);
  }
};
